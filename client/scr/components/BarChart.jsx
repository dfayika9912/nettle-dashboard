...
const BarChart = ({ data, view, year }) => {
  ...
  const downloadCSV = () => {
    const csvData = [
      ["State", "Probability (%)"],
      ...data.map((d) => [d.state_full, d.predictions[view].toFixed(2)]),
    ];
    ...
    link.setAttribute("download", `natural_disaster_risk_${view}_${year}.csv`);
  ...
  const downloadPDF = () => {
    ...
    img.onload = () => {
      ...
      pdf.save(`natural_disaster_risk_${view}_${year}.pdf`);
    };
